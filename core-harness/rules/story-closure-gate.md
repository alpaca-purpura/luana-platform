...
ready → developing → developed ─[G Chris-verify]─[R reconcile]→ reviewing → done
                         │                                          └─ APPROVED→/pm-{brand} merge
                         └─ default: pausa-y-ofrece (phase: AWAIT_CHRIS_VERIFY)
                            autonomous_mode: true → corre a /auditor sin pausa
```

`/dev-team` cierra `developed` → **G (Chris-verify)** salvo `autonomous_mode: true` → **R (reconcile /pm-{brand})** → AUTO-HANDOFF `/auditor`. APPROVED → `/pm-{brand}` merge. Sin `defer_audit: true` el gate es ABSOLUTO.
...
| F — MERGE | `/pm-{brand}` | `reviewing → done` · REFUSE sin `chris_verify.signoff` (funcional) · 07-merge.md · archive story R2 |
...
reconciled: false                  # /pm-{brand} lo pone true en R (precondición que el auditor LEE antes de empezar)
```
