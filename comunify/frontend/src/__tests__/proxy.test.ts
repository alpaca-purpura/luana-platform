/**
 * Unit tests — proxy.ts (Comunify, replicado de vitalia 2026-05-20)
 *
...
// __dirname = comunify/frontend/src/__tests__
// ROOT = comunify/frontend (2 levels up)
const ROOT = resolve(__dirname, "../..");
const PROXY_PATH = join(ROOT, "src", "proxy.ts");

describe("Comunify FE — Proxy Clerk", () => {
  it("proxy.ts exists at src/proxy.ts", () => {
    expect(
      existsSync(PROXY_PATH),
      "proxy.ts no encontrado en comunify/frontend/src/proxy.ts"
    ).toBe(true);
  });
  ...
  it("proxy.ts configura rutas públicas esperadas (sign-in, sign-up, public, webhooks, health)", () => {
    ...
    expect(source).toContain("/api/v1/comunify/webhooks");
