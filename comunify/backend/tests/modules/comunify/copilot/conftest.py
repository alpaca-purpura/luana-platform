"""Env defaults for the comunify copilot mount tests.

The engine copilot ``/chat`` router (``luana_core_copilot.api.chat``) imports
``luana_core_platform.core.config`` at module load, which instantiates
``Settings()`` and requires ~16 env vars. These must be set BEFORE any import
triggers config loading.

Canonical pattern mirrored from
``core/luana-core-copilot/tests/conftest.py`` (CI test env defaults — dummy
