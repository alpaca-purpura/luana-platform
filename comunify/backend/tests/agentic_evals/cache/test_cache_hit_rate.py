"""Agentic eval — V-AE-27: cache hit rate ≥85% on slots 1-6.
...
Spec sources:
  ...
  * 04-validators.yaml::V-AE-27 threshold cache_hit_rate_min: 0.85
  * 06-tickets.yaml::T-prompts-1
  * Pattern reference: vitalia tests/agentic_evals/cache/test_cache_hit_rate.py
...
# ─────────────────────────────────────────────────────────────────────────────
# Story 12 fixture tenants (per 02-design § 11.2 — 3 distilled voices)
# ─────────────────────────────────────────────────────────────────────────────

# Three Story-12 fixture tenants with distinct voices (Slot 5 BRAND_VOICE).
# Slots 1-4, 6 invariant cross-tenant; Slot 5 varies per tenant. Within-tenant
# turns share Slot 5 (cache hit on slot 5 too).
_FIXTURE_TENANTS = [
    {
        "tenant_id": "anabella-conexion-ar-uuid",
        "channel": "whatsapp",
        "brand_voice": (
            "# Voz Anabella Conexión AR (cálida vehemente, voseo distilled)\n\n"
            "Hablás con calidez vehemente y validación constante. Voseo "
            "argentino distilled: 'mirá', 'tenés', 'querés', 'vos podés', "
            "'te abrazo'. Acompañás business coaches mujeres con narrativa "
            "del paso a paso. Anchors: 'no estás sola', 'vamos paso a paso', "
            "'vos podés'. Tono profesional pero cercano como una amiga "
            "experta que también caminó el camino."
        ),
    },
    {
        "tenant_id": "trini-nutricion-cl-uuid",
        "channel": "whatsapp",
        "brand_voice": (
            "# Voz Trini Nutrición Real CL (validadora antidogmática, tuteo neutro chileno)\n\n"
            "Tono validador, didáctico, antidogmático. Tuteo neutro chileno: "
            "'tú', 'mira', 'sin culpa'. NO voseo. Acompañas a mujeres en "
            "relación con la comida desde la libertad alimentaria. Anchors: "
            "'te entiendo perfecto', 'sin culpa', 'hacé las paces' (el imperativo "
            "AR queda en chileno como 'haz las paces')."
        ),
    },
    {
        "tenant_id": "pablo-productividad-mx-uuid",
        "channel": "whatsapp",
        "brand_voice": (
            "# Voz Pablo Productividad MX (directo técnico-accesible, tuteo neutro broad LatAm)\n\n"
            "Tono directo, sin filler, técnico-accesible. Tuteo neutro LatAm "
            "broad. Estructura clara, frases cortas, ejemplos concretos. "
            "Apto para públicos panregionales — evita voseo y léxico mexicano "
            "específico. Anchors: 'claridad sobre énfasis', 'ejemplos "
            "concretos', 'respeto tu tiempo'."
        ),
    },
]
...
# ─────────────────────────────────────────────────────────────────────────────
# V-AE-27 — cache hit rate ≥85% across simulated multi-turn conversations
# ─────────────────────────────────────────────────────────────────────────────
...
def test_cache_hit_rate_target_85_percent_min() -> None:
    """V-AE-27: simulated cache hit rate on slots 1-6 ≥85% across 3 tenants × 10 turns.
    ...
    assert hit_rate >= 0.85, (
        ...
        f"Run test_comunify_no_pii_in_cacheable_slots.py to localize."
    )


def test_cross_tenant_cache_isolation_slot_5_varies() -> None:
    """V-AE-27 corollary: Slot 5 BRAND_VOICE differs per tenant (cache_key isolates).

    Validates that two distinct tenants under their OWN cache_keys do NOT
    cross-contaminate hits. Anabella's voice should not register as a cache
    hit when Trini queries (different prompt_cache_key bucket), even though
    slots 1-4 + 6 are byte-equal.
    """
    stats = _SimulatedCacheStats()

    anabella = _FIXTURE_TENANTS[0]
    trini = _FIXTURE_TENANTS[1]

    # Turn 1: Anabella — all 6 slots are creations (first time seen).
    anabella_blocks = cacheable_prefix_blocks(channel=anabella["channel"], brand_voice_compiled=anabella["brand_voice"])
    stats.record_turn(
        cache_key=prompt_cache_key(anabella["tenant_id"]),
        cacheable_blocks=anabella_blocks,
    )
    creations_after_anabella_t1 = stats.cache_creation_input_tokens
    assert creations_after_anabella_t1 > 0
    assert stats.cache_read_input_tokens == 0

    # Turn 2: Trini — under its OWN cache_key, all 6 slots are creations
    # too (different bucket). NO cross-tenant cache hit.
    trini_blocks = cacheable_prefix_blocks(channel=trini["channel"], brand_voice_compiled=trini["brand_voice"])
    stats.record_turn(
        cache_key=prompt_cache_key(trini["tenant_id"]),
        cacheable_blocks=trini_blocks,
    )
    assert stats.cache_creation_input_tokens > creations_after_anabella_t1, (
        "Trini turn must register as creation (new cache bucket) — got "
        f"creations unchanged at {stats.cache_creation_input_tokens}. "
        "Cross-tenant isolation broken: Anabella's blocks should NOT count as "
        "cache hits for Trini (different prompt_cache_key)."
    )
    assert stats.cache_read_input_tokens == 0, (
...
def test_within_tenant_repeated_turn_full_cache_hit() -> None:
    """V-AE-27 cement: within one tenant, 2nd turn = 100% cache hit on slots 1-6.
    ...
    # Turn 1
    blocks = cacheable_prefix_blocks(channel=anabella["channel"], brand_voice_compiled=anabella["brand_voice"])
    stats.record_turn(cache_key=cache_key, cacheable_blocks=blocks)
    ...
    blocks_2 = cacheable_prefix_blocks(channel=anabella["channel"], brand_voice_compiled=anabella["brand_voice"])
    ...
    # turn + 1 read turn). Real V-AE-27 threshold is per session aggregate
    # (10 turns → 90%); this micro-test just asserts the per-block determinism.


def test_prompt_cache_key_canonical_str() -> None:
    """V-AE-27 sanity: prompt_cache_key coerces to str regardless of input type."""
    import uuid

    tid_uuid = uuid.uuid4()
    tid_str = "anabella-conexion-ar-uuid"
    tid_int = 42
