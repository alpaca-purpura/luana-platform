...
Per Story 12 T-prompts-1 acceptance:
  No `{tenant_name}` interpolated mid-block, no lead/member PII (name/phone/
  email), no timestamps, no random IDs, no conversation_id in slots 1-4 + 6
  (cacheable cross-tenant). Slot 5 (BRAND_VOICE) is per-tenant — varies
  legitimately. Slot 7-10 NOT cached — exempt.
...
Spec sources:
  ...
  * Pattern reference: vitalia tests/architecture/test_vitalia_no_pii_in_cacheable_slots.py

Validates the LITERAL TEMPLATE TEXT — not runtime substitution. The dynamic
placeholders that LIVE in slot 4 (e.g. `{brand_name}`, `{creator_name}`,
`{creator_email}`, `{emergency_line_by_country}`) are LLM-side substitution
markers (model fills at generation time using slot 8 task-specific data). They
do NOT get Python-side interpolated pre-compose, so they preserve cache prefix
...
_FORBIDDEN_DYNAMIC_TOKENS = [
    ...
    "{lead_phone}",  # lead PII
    "{lead_email}",  # lead PII
    "{member_name}",  # member PII
    "{member_phone}",  # member PII
    "{member_email}",  # member PII
    "{subscriber_name}",  # subscriber PII
    "{subscriber_phone}",  # subscriber PII
    "{subscriber_email}",  # subscriber PII
    "{cohort_member_name}",  # community member PII (D16 doxxing prevention)
    "{dni}",  # national IDs LatAm
    "{rfc}",  # Mexico
    "{rut}",  # Chile
    "{curp}",  # Mexico
]
...
# the literal `{brand_name}` text is byte-identical cross-tenant.
_ALLOWED_LLM_SIDE_MARKERS = {
    "{brand_name}",  # the Comunify brand label (LLM-substituted at gen-time)
    "{creator_name}",  # per-tenant creator name (LLM-substituted at gen-time)
    "{creator_email}",  # creator contact (LLM-substituted at gen-time)
    "{emergency_line_by_country}",  # emergency hotline per member's country
}
...
def _all_cacheable_slot_texts() -> dict[str, str]:
    ...
    tenant from personality_profiles.system_instruction (distilled v2).
    ...
    return {
        ...
        "slot_4_community_safety_rails": load_slot_4_community_safety_rails(),
...
def test_no_email_addresses_in_cacheable_slot(slot_name: str, slot_text: str) -> None:
    ...
    assert not matches, (
        ...
        f"Lead/member/creator emails are PII — per 02-design § 10.4 forbidden in "
        f"cache prefix. Move dynamic emails to Slot 8 task-specific (NOT cached) "
        f"or sanitize via shared sanitize_payload."
    )
...
def test_no_phone_numbers_in_cacheable_slot(slot_name: str, slot_text: str) -> None:
    ...
    assert not real_matches, (
        ...
        f"Lead/member/creator phones are PII — per 02-design § 10.4 forbidden in "
        f"cache prefix. Move dynamic phones to Slot 8 task-specific (NOT cached)."
    )
...
def test_slot_4_uses_only_allowed_llm_side_markers() -> None:
    """A2: Slot 4 LLM-side substitution markers (`{brand_name}` etc.)
...
def test_cacheable_prefix_byte_equal_across_distinct_tenants() -> None:
    ...
    """
    anabella_voice = "Voz Anabella Conexión AR (cálida, voseo distilled): 'te abrazo, vos podés'"
    trini_voice = "Voz Trini Nutrición CL (tuteo neutro chileno): 'te entiendo perfecto, sin culpa'"

    anabella_blocks = cacheable_prefix_blocks(channel="whatsapp", brand_voice_compiled=anabella_voice)
    trini_blocks = cacheable_prefix_blocks(channel="whatsapp", brand_voice_compiled=trini_voice)
    ...
    for idx in invariant_indices:
        assert anabella_blocks[idx]["text"] == trini_blocks[idx]["text"], (
            f"Slot at index {idx} differs between Anabella vs Trini tenants. "
    ...
    assert anabella_blocks[4]["text"] != trini_blocks[4]["text"], (
