"""Architecture fitness gate — vertical-creator-economy-fidelity rubric MD v1 schema (Story 12 T-rubric-1).

Story 12 cement: ``docs/specs/rubrics/vertical-creator-economy-fidelity.md`` declares
the v1 rubric for creator-economy-vertical agentic eval. The rubric is consumed by:

  * Story 12 grader runtime (T-eval-1, downstream)
...
3. Frontmatter declares ``id: vertical-creator-economy-fidelity``.
...
7. Frontmatter declares ``verticals`` covering creator_economy.
8. Frontmatter declares ``threshold_default: 0.85`` (community safety bar).
9. Frontmatter declares non-empty ``ssot`` list referencing Story 12 spec + rules.
10. Frontmatter declares ``owner_story: luana-comunify-bootstrap``.
11. Body documents EXACTLY 5 assertions (A1..A5) per spec § 13.
...
_COMUNIFY_RUBRIC_PATH: Path = _RUBRICS_DIR / "vertical-creator-economy-fidelity.md"

# ---------------------------------------------------------------------------
# Expected schema constants (v1 cement)
# ---------------------------------------------------------------------------

_EXPECTED_RUBRIC_ID = "vertical-creator-economy-fidelity"
_EXPECTED_RUBRIC_VERSION = 1
_EXPECTED_THRESHOLD_DEFAULT = 0.85
_EXPECTED_OWNER_STORY = "luana-comunify-bootstrap"
_EXPECTED_MODULES: frozenset[str] = frozenset({"sales_agent", "copilot"})
_EXPECTED_APPLIES_TO: frozenset[str] = frozenset({"agentic-story"})
# verticals must contain at least creator_economy (spec § 13)
_EXPECTED_VERTICALS_SUBSET: frozenset[str] = frozenset({"creator_economy"})
...
def _read_rubric_text() -> str:
    ...
    assert _COMUNIFY_RUBRIC_PATH.is_file(), (
        f"vertical-creator-economy-fidelity.md not found at {_COMUNIFY_RUBRIC_PATH}. "
        "Story 12 T-rubric-1 deliverable absent."
    )
    return _COMUNIFY_RUBRIC_PATH.read_text(encoding="utf-8")
...
def _extract_frontmatter(text: str) -> dict[str, object]:
    ...
    Recognized formats:
      A) Pure frontmatter at top of file (Pandoc default):
            ---
            id: ...
            ---
            (body)
      B) Embedded YAML code fence (Story E pattern):
            ```yaml
            ---
            id: ...
            ---
    ...
    """
    # Try Format A first (pure frontmatter at top)
    ...
    # Try Format B (yaml code fence with --- inside)
    ...
    raise AssertionError(
        "Could not extract YAML frontmatter from vertical-creator-economy-fidelity.md. "
        "Expected either pure top-of-file frontmatter delimited by --- lines, "
        "or an embedded ```yaml ... ``` code block."
    )
...
def test_comunify_rubric_md_file_exists() -> None:
    """``docs/specs/rubrics/vertical-creator-economy-fidelity.md`` MUST exist."""
    assert _COMUNIFY_RUBRIC_PATH.is_file(), (
        f"vertical-creator-economy-fidelity.md not found at {_COMUNIFY_RUBRIC_PATH}. "
        "Story 12 T-rubric-1 deliverable absent."
    )
...
def test_comunify_rubric_md_is_non_empty() -> None:
    """Rubric MD MUST be non-empty."""
    text = _read_rubric_text()
    assert text.strip(), "vertical-creator-economy-fidelity.md is empty"
...
def test_comunify_rubric_frontmatter_extractable() -> None:
...
def test_comunify_rubric_frontmatter_id() -> None:
    """Frontmatter ``id`` MUST equal ``vertical-creator-economy-fidelity``."""
...
def test_comunify_rubric_frontmatter_version_is_1() -> None:
...
def test_comunify_rubric_frontmatter_applies_to_includes_agentic_story() -> None:
...
def test_comunify_rubric_frontmatter_modules_includes_sales_agent_and_copilot() -> None:
...
def test_comunify_rubric_frontmatter_verticals_covers_creator_economy() -> None:
    """Frontmatter ``verticals`` MUST cover at least creator_economy."""
    fm = _extract_frontmatter(_read_rubric_text())
...
def test_comunify_rubric_frontmatter_threshold_default_is_0_85() -> None:
    """Frontmatter ``threshold_default`` MUST equal ``0.85`` (community safety bar)."""
    fm = _extract_frontmatter(_read_rubric_text())
    ...
    assert actual == _EXPECTED_THRESHOLD_DEFAULT, (
        ...
        "0.85 is the community safety bar per spec § 13."
    )


def test_comunify_rubric_frontmatter_ssot_non_empty_list() -> None:
    """Frontmatter ``ssot`` MUST be a non-empty list referencing Story 12 SSoT."""
    fm = _extract_frontmatter(_read_rubric_text())
    actual = fm.get("ssot")
    ...
    assert (
        ...
        or "Story 12" in text_concat
        or "story-12" in text_concat.lower()
    ), f"frontmatter ssot entries should reference Story 12 spec/rules; got {actual!r}"


def test_comunify_rubric_frontmatter_owner_story_is_luana_comunify_bootstrap() -> None:
    """Frontmatter ``owner_story`` MUST equal ``luana-comunify-bootstrap``."""
    fm = _extract_frontmatter(_read_rubric_text())
    actual = fm.get("owner_story")
    assert actual == _EXPECTED_OWNER_STORY, (
        f"frontmatter owner_story={actual!r} (expected {_EXPECTED_OWNER_STORY!r})"
    )
...
def test_comunify_rubric_body_has_5_assertions_a1_through_a5() -> None:
...
def test_comunify_rubric_each_assertion_declares_weight() -> None:
...
def test_comunify_rubric_assertion_weights_sum_to_1_00() -> None:
...
def test_comunify_rubric_body_documents_scoring_formula() -> None:
...
def test_comunify_rubric_body_documents_threshold_0_85() -> None:
    """Body MUST document the threshold ``≥0.85`` (community safety bar)."""
    text = _read_rubric_text()
    has_threshold = bool(re.search(r"≥\s*0\.85|>=\s*0\.85|threshold.*0\.85", text, re.IGNORECASE))
    assert has_threshold, "Body MUST document the threshold value 0.85 (e.g. 'Threshold ≥0.85')."


def test_comunify_rubric_body_documents_cache_invalidation_rule() -> None:
    ...
    has_invalidation_text = bool(
        re.search(r"rubric_version.*?invalidat", text, re.IGNORECASE | re.DOTALL)
    )
...
def test_comunify_rubric_body_documents_passk_thresholds() -> None:
    """Body SHOULD document the pass^k thresholds for all persona_kind categories."""
    text = _read_rubric_text()
    has_thresholds = "0.75" in text or "0.95" in text or "0.85" in text
    assert has_thresholds, (
        "Body SHOULD reference pass^k thresholds. Spec § 13 documents these."
    )
