"""Comunify Clerk webhook adapter — user.created → creator tenant onboarding.

Per 03-arch-be.md T-be-9:
- Verifies Clerk webhook HMAC-SHA256 via Svix-signature header.
- Idempotent: same svix_id → caller checks dedup before dispatching.
- Calls OnboardingService.create_creator_profile when event=user.created.
...
Secret env var: COMUNIFY_CLERK_WEBHOOK_SECRET.

Replay attack protection:
  - Timestamp tolerance ±5 min (300s).
  - svix_id dedup: caller checks _seen_event_ids in webhook_routes.

# [COMUNIFY-T-BE-9-WEBHOOK-IDEMPOTENCY-CLERK]
"""
...
class ClerkWebhookAdapter:
    ...
    Args:
        webhook_secret: Raw secret from COMUNIFY_CLERK_WEBHOOK_SECRET env var.
    ...
    def verify(
        ...
        *,
        ...
        if not self.webhook_secret:
            raise ValueError("webhook_secret is empty — set COMUNIFY_CLERK_WEBHOOK_SECRET env var")
        ...
        logger.info(
            "comunify_clerk_webhook_verified",
            event_id=svix_id,
    ...
    def from_env(
        cls,
        *,
        webhook_secret_env: str = "COMUNIFY_CLERK_WEBHOOK_SECRET",
    ) -> "ClerkWebhookAdapter":
...
__all__ = ("ClerkWebhookAdapter", "ClerkWebhookEvent")
