"""Comunify ManyChat webhook adapter — IG DM inbound → sales_agent dispatch.

Per 03-arch-be.md T-be-9:
...
HMAC algorithm:
  X-MC-Signature = HMAC-SHA256(raw_body, COMUNIFY_MANYCHAT_WEBHOOK_SECRET)
  The header value is the hex digest (lowercase).

Replay protection:
  - Timestamp tolerance ±5 min if ``timestamp`` field present in payload.
  - Dedup key: (subscriber_id, message_id) — caller tracks in _seen_event_ids.

# [COMUNIFY-T-BE-9-WEBHOOK-IDEMPOTENCY-MANYCHAT]
"""
...
class ManychatWebhookAdapter:
    ...
    Args:
        webhook_secret: Secret from COMUNIFY_MANYCHAT_WEBHOOK_SECRET env var.
    ...
    def verify(
        ...
        *,
        ...
        if not self.webhook_secret:
            raise ValueError("webhook_secret is empty — set COMUNIFY_MANYCHAT_WEBHOOK_SECRET env var")
        ...
        if ts_value is not None:
            try:
                ...
                # Re-raise only tolerance violations, skip bad-ts silently
        ...
        if not subscriber_id:
            logger.warning("comunify_manychat_webhook_missing_subscriber_id", payload_keys=list(payload.keys()))

        logger.info(
            "comunify_manychat_webhook_verified",
    ...
    def from_env(
        cls,
        *,
        webhook_secret_env: str = "COMUNIFY_MANYCHAT_WEBHOOK_SECRET",
...
__all__ = ("ManychatInboundEvent", "ManychatWebhookAdapter")
