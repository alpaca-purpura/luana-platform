 ...
 * nicolify/frontend/src should be page-primitives (canon §2.7, ADR-014).
 * A `<div className="flex-col gap-…">` or `<div className="grid-cols-…">` is
 * layout that belongs in a primitive (Section / PageContentStack / grid).
 *
 * SHRINK-ONLY ratchet:
 ...
 *
 * Baseline MEASURED 2026-06-24 (HB-106 seed): 39 layout <div> across 11 files.
 * These are pre-existing; the studio adoption story (nicolify-r0-design-system-adoption)
 * migrates them to primitives. The lock only blocks NEW raw layout divs.
 *
 * Detection (parity with vitalia _ds-lock-scanner::isLayoutDiv): className contains
