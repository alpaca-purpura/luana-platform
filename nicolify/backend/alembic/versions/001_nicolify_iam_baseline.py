"""Nicolify — schema-mirror of luana-core-iam engine tables.
...
Promotion proposal reference:
    ...
    (state: accepted) — nicolify adopts the same engine (AD-3).
...
CERO tablas nicolify_* (precavido — brand tables nascent in future stories).
Pattern cloned verbatim from:
    vitalia/backend/alembic/versions/022_vitalia_add_engine_iam_tables.py

Revision ID: 001_nicolify
Revises: None  (baseline — reemplaza legacy 001_initial_snapshot visionarias)
...
def upgrade() -> None:
    """Create engine IAM tables idempotent (schema-mirror for nicolify brand)."""
