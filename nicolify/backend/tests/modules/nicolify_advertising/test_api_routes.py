...
from src.modules.nicolify.advertising.api.routes import router
from src.modules.nicolify.advertising.application.dto.association_dto import (
    AssociationSuggestionDTO,
)
from src.modules.nicolify.advertising.application.dto.health_check_dto import (
    MetaHealthCheckDTO,
)
from src.modules.nicolify.advertising.application.dto.metrics_by_offer_dto import (
...
class TestAutoDetectEndpoint:
    def test_auto_detect_delegates_to_service(self, db, tenant_id):
        ...
        with patch(
            "src.modules.nicolify.advertising.api.routes.OfferDetectionService",
        ) as svc_cls:
            instance = MagicMock()
...
class TestHealthCheckEndpoint:
    def test_delegates_to_service(self, db, tenant_id):
        ...
        with patch("src.modules.nicolify.advertising.api.routes.HealthCheckService") as svc_cls:
            instance = MagicMock()
...
class TestMetricsByOfferEndpoint:
    def test_delegates_and_validates_period(self, db, tenant_id):
        ...
        with patch(
            "src.modules.nicolify.advertising.api.routes.MetricsByOfferService",
        ) as svc_cls:
            instance = MagicMock()
            instance.run = AsyncMock(return_value=dto)
            svc_cls.return_value = instance
...
class TestCampaignTemplateEndpoint:
    def test_returns_404_when_no_template(self, db, tenant_id):
        client = _build_client(db, tenant_id)
        with patch(
            "src.modules.nicolify.advertising.api.routes.CampaignTemplateService",
        ) as svc_cls:
            instance = MagicMock()
