...
def test_codeowners_file_exists():
    ...
    assert CODEOWNERS_PATH.is_file(), (
        f"CODEOWNERS not found at {CODEOWNERS_PATH}. "
        "Anti-island gate #1 requires CODEOWNERS to protect core paths."
    )
...
def test_codeowners_protects_core_copilot():
    ...
    assert "core/copilot/**" in content, (
        "CODEOWNERS is missing rule for 'core/copilot/**'. "
        "All core AI module paths must be protected."
    )
...
def test_codeowners_protects_core_sales_agent():
    ...
    assert "core/sales-agent/**" in content, (
        "CODEOWNERS is missing rule for 'core/sales-agent/**'. "
        "All core AI module paths must be protected."
    )
...
def test_codeowners_protects_core_shared():
    ...
    assert "core/shared/**" in content, (
        "CODEOWNERS is missing rule for 'core/shared/**'. "
        "All core shared abstractions must be protected."
    )
