...
def test_adr_readme_mentions_michael_nygard_format():
    ...
    assert has_nygard, (
        "ADR README does not mention 'Michael Nygard' or 'template'. "
        "The README must explain the ADR format."
    )
...
def test_adr_readme_has_index():
    ...
    assert has_index, (
        "ADR README does not contain an index (índice). "
        "The README must list all ADRs in an index table."
    )
...
def test_adr_folder_exists():
    ...
    assert adr_dir.is_dir(), (
        f"ADR directory not found at {adr_dir}. "
        "The ADR directory must exist for governance to function."
    )
