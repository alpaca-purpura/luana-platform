...
kit_alignment_2026_06_24:
  ...
  next: "DECISIÓN Chris (2026-06-24): Opción B — agregar la primitiva de micro-layout (Stack/Flex + Grid) a @luana/ui-kit AHORA + scanner HB-106 altitude-aware, LUEGO migrar los 14 divs de abel → abel 100% canon-clean. Core/cross-brand = engine boundary → auditor Caso D ESCALATE /pm-luana (promotion gate). abel APPROVED (kit-atom) pero CIERRE DIFERIDO hasta que la primitiva del kit aterrice."
  blocked_on_kit_primitive: "core/@luana/ui-kit micro-layout primitive (Stack/Flex + Grid) + scanner altitude-aware (HB-111). Owner /pm-luana (promotion gate · cross-brand). Cuando aterrice: builder-frontend migra los 14 abel divs contra la primitiva nueva → baseline 32→18 → demo gate #37 → merge."
