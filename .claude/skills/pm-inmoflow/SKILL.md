...
dentro de los args del `/pm-inmoflow`, o el contexto determina la skill
...
3. `Skill(skill: "<name>", args: "inmoflow {story-id}")` inline
...
Cuando una story inmoflow transiciona a `status: live` / `done`, `/pm-inmoflow` MUST ejecutar el paso 2 del
capability promotion (R32) ANTES de cerrar la sesión:

1. Para cada feature shipped en la story → escribir `inmoflow/docs/product/capabilities/{module}/{cap}.yaml`
...
- `new` → crear `inmoflow/docs/product/capabilities/{module}/{cap_slug}.yaml` con schema completo + change_log[0] type=new + scenarios iniciales
...
En Fase F (merge a `done`), `/pm-inmoflow` REFUSE si:
