...
| "idea {x}" | **Primero corré el intake-handshake (§ arriba)** — conversación de diseñador del sistema (zona/caja + extiende-o-nuevo + qué ya existe + empujás). RECIÉN de esa conversación creás el story dir `state=idea` con **2 archivos juntos**: `lupulo/docs/product/stories/{slug}/checkpoint.md` + `chris-input.md` (desde `docs/specs/templates/00-chris-input-template.md` — libro mayor donde TODO lo que Chris pidió en la conversación de creación queda registrado; Claude lo puede rebatir durante el ciclo de vida) |
...
**Regla cardinal:** si Chris nombra explícitamente una skill secundaria (`/po-ux`, `/po`, `/ux-agentico`, `/architect`, `/dev-team`, `/auditor`) dentro de los args del `/pm-lupulo` (o cualquier `/pm-{brand}`), o el contexto del turno determina que el siguiente paso obvio es una de esas skills, **invocá `Skill` tool inline en el mismo turn post-Step 0**. NO devuelvas handoff textual.
...
❌ Caso real 2026-05-23 F1-S4 (vitalia): post-Step 0 GREEN el PM devolvió "Chris, invocá /po-ux ..." esperando re-tipeo → estancamiento. ✅ Fix: invocar `Skill(skill: "po-ux", args: "lupulo {story-id}")` directamente. Ver `.claude/rules/pm-skill-chaining.md`.
...
- `.claude/rules/sistema-docs-schema.md` — R1+R2+R3 schema enforcement `lupulo/docs/` (cement 2026-05-19)
