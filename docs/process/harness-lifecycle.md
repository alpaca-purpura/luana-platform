...
| **Mensual/trimestral (o "huele a drift")** | `/harness-audit` full → catálogo fresco que realimenta el backlog |
...
## 8. Auditoría periódica — `/harness-audit`
...
- `/harness-audit` saved workflow (auditoría periódica).
- Extender `learning-detect.sh` para sugerir capturas de harness post-Edit/Bash.
- `memory: project` en agents auditores (acumulan anti-patterns del harness cross-sesión).
