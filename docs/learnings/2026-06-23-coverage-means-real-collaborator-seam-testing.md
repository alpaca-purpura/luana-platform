...
2. **`/architect` · `04-validators.yaml § test_construction_plan`** — cada escenario declara la **COSTURA que cruza** + el **tipo de test real-collaborator requerido** (real-DB / contract / router / E2E-live / live-verify). No se permite "unit (mocked)" para un escenario de costura. Schema nuevo: `scenario_to_test: { sc: SC-x, seam: code-db|fe-be|router|auth|component-shell, test_type: integration-realdb|contract|router|e2e-live|live-verify, target: <path> }`.
