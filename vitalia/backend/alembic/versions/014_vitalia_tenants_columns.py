...
def upgrade() -> None:
    ...
    op.execute("ALTER TABLE tenants ADD COLUMN IF NOT EXISTS is_onboarded BOOLEAN NOT NULL DEFAULT FALSE;")

    # location_country — ISO 3166-1 alpha-2 (AR, PE, MX, CO, CL, BR, US, ES, ...)
    op.execute("ALTER TABLE tenants ADD COLUMN IF NOT EXISTS location_country VARCHAR(2);")

    # location_city — disambiguation + analytics regional
    op.execute("ALTER TABLE tenants ADD COLUMN IF NOT EXISTS location_city VARCHAR(255);")

    # timezone — IANA TZ database string ('America/Lima', 'America/Argentina/Buenos_Aires')
    op.execute("ALTER TABLE tenants ADD COLUMN IF NOT EXISTS timezone VARCHAR(64);")

    # Backfill: tenants created before 2026-05-17 are treated as onboarded
    # (they completed setup in previous brand versions — is_onboarded=TRUE)
    op.execute("""
        UPDATE tenants
