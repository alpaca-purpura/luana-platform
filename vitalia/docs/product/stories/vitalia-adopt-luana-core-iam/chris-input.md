...
Story migrada al schema v2: `release=F0`, `cap_target=luana-core-adoption`, `cap_change_type=new`. Legacy `outcome` + `phase` preservados durante deprecation gradual.

Cuando estés listo para refinar, llena 💭 Notas + 📎 Referencias arriba e invoca `/po-ux vitalia vitalia-adopt-luana-core-iam` (o `/po` si es service story, o `/ux-agentico` si es agentic).
