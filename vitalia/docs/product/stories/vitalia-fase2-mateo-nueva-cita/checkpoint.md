...
verify_battery_2026_06_24:                          # /dev-team VERIFY_BATTERY — correr TODA la batería del architect ANTES del auditor (Chris opt-A). Estado por gate:
  ...
  e2e_smoke: "🟡 4/5 funcional GREEN (happy-path + form-render + back-pill). a11y destapó 4 violaciones (ver a11y_verified)."
  e2e_regression: "🟡 ERA HUÉRFANO (e2e/specs/regression/ no matcheaba ningún project testMatch → los 9 SC NUNCA corrieron = cobertura fantasma, HB-96). MOVIDO a e2e/regression/mateo/. 8/11; los 3 rojos son dev-flake (SC-crear-paciente form not-found) / engine-a11y-ya-fixed / SC-mini-vista hidden (triage pendiente)."
  a11y_verified: "✅ 0 VIOLACIONES axe wcag2aa (probado robusto manual-poll 3×). 4 violaciones reales encontradas+arregladas: (1) aria-valid-attr-value CanalPicker (Tabs sin TabsContent → radiogroup); (2) button-name ServicePicker/DoctorPicker (SelectTrigger sin aria-label); (3) scrollable-region-focusable ChatMessages @luana/ui-kit (tabIndex={0}, vía /pm-luana ratificado Chris); (4) color-contrast CanalPicker inactivo (regresión propia: text-muted-foreground sobre bg-muted → text-foreground/80)."
  prod_build_e2e: "❌ BLOQUEADO por deuda PRE-EXISTENTE no relacionada: `next build` (host) falla en SSG-prerender de páginas ajenas — /marketing (acoplamiento env Clerk en build) + /onboarding/wizard (useSearchParams sin <Suspense> = CSR-bailout) + probablemente más (build sale al 1er error). El FE NUNCA se prod-buildeó limpio en host. Es remediación aparte (bugfix/HB), no scope de mateo. a11y se verificó robusto sin prod-build."
  e2e_dev_flake: "`next dev --webpack` resetea intermitente chunks de boundary (error.js/not-found.js) → SyntaxError → rompe el client shell en el gate anti-burbuja (falsos rojos). El shell es SANO (dsr='true' en aislamiento). NO es producto. Pendiente: e2e contra prod-build (bloqueado, ver arriba) o estabilizar dev. HB pendiente."
  ...
  ci_parity: "Docker (`--target test`) DEFERIDO por diseño (sentinel .ci-parity-deferred + Dockerfile sin stage test). Gate dev-phase real = NATIVO, corrido: BE ruff ✅ · FE eslint ✅ · FE vitest ✅ 2662/2662 · BE scheduling+crm ✅ 632. ÚNICOS rojos = ajenos a mateo: 5 arch-fails + 2 collection-errors TODOS de sales_agent EP3/inbound-seam + pgcrypto = WIP no-commiteado de la story hermana adrian-canal-inbound (luana_core_sales_agent.inbound_mode_seam módulo ausente, pre-existente). Cero código sales_agent tocado por mateo → no son regresiones de esta story."
  next: "a11y ✅ + engine fix ✅ + ci-parity nativo. Pendiente: (a) triage SC-mini-vista hidden; (b) decisión prod-build-e2e (remediación CSR-bailout aparte) o aceptar dev-flake con retries. Con eso → G (Chris)."
