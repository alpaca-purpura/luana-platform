...
import type { UploadMedicalPdfRequest, UploadMedicalPdfResponse } from "../types/treatment.types";
...
export function usePatientUploadPdf(patientId: string) {
  ...
  return useMutation({
    mutationFn: async (data: UploadMedicalPdfRequest): Promise<UploadMedicalPdfResponse> => {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: vitaliaQueryKeys.patients.detail(patientId) });
    },
  });
}
