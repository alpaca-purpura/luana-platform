...
import type { ComplianceSeverity, ComplianceEventItem } from "@/features/vitalia/types/compliance.types";
...
function countBySeverity(events: ComplianceEventItem[], severity: ComplianceSeverity): number {
  return events.filter((e) => e.severity === severity).length;
}

function StatCardItem({ label, value, variant, isLoading }: StatCard & { isLoading?: boolean }) {
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        <div className="h-8 w-16 rounded bg-gray-200 animate-pulse" aria-hidden="true" />
      ) : (
...
export function ComplianceStatsCards({
  ...
  return (
    <section className={cn("space-y-4", className)} aria-label={MICROCOPY_COMPLIANCE.title}>
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-semibold text-gray-900">
            {MICROCOPY_COMPLIANCE.title}
          </h2>
          <p className="text-sm text-gray-500">{MICROCOPY_COMPLIANCE.subtitle}</p>
        </div>
      </div>
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4 space-y-2">
          ...
          {Object.entries(MICROCOPY_COMPLIANCE.eventTypes).map(([key, label]) => {
            const count = events.filter((e) =>
              e.event_type === key || e.event_type.includes(key)
            ).length;
            ...
            return (
              <div key={key} className="flex items-center gap-2">
                ...
                <div className="w-24 h-1.5 bg-gray-100 rounded-full overflow-hidden" aria-hidden="true">
                  <div
      ...
      {!isLoading && events.length === 0 && (
        <div className="rounded-lg border border-dashed border-gray-200 p-8 text-center">
          <p className="text-sm text-gray-500">Sin eventos de cumplimiento registrados.</p>
        </div>
      )}
    </section>
  );
