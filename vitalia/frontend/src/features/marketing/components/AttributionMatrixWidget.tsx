...
const KPI_COLUMNS: KpiColumn[] = [
  { key: "leads", label: MARKETING_COPY.attribution.leadsLabel, isConvRate: false },
  { key: "qualified", label: MARKETING_COPY.attribution.qualifiedLabel, isConvRate: true, denominator: "leads" },
  { key: "convListo", label: MARKETING_COPY.attribution.convListoLabel, isConvRate: true, denominator: "qualified" },
  { key: "reservations", label: MARKETING_COPY.attribution.reservationsLabel, isConvRate: true, denominator: "convListo" },
  { key: "adoption", label: MARKETING_COPY.attribution.adoptionLabel, isConvRate: true, denominator: "reservations" },
];
 ...
 */
export const AttributionMatrixWidget = forwardRef<HTMLDivElement, AttributionMatrixWidgetProps>(
  ({ period = "30d", className }, ref) => {
    ...
    const rows: RowData[] = [
      ...data.origins.map((row) => ({
        label: originLabelMap[row.origin as keyof typeof originLabelMap] ?? row.origin,
        row,
        isTotal: false,
      })),
      {
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {rows.map(({ label, row, isTotal }) => (
                ...
                >
                  <td className="px-4 py-2 vt-text whitespace-nowrap">
                    {label}
                  </td>
                  {KPI_COLUMNS.map((col) => {
                    const rawValue = row[col.key] as number;
                    const rate = getRate(row, col);
                    const colorClass = col.isConvRate ? convRateColorClass(rate) : "vt-text";
                    ...
                    return (
                      <td
                        key={col.key}
                        className={cn("px-3 py-2 text-right tabular-nums", colorClass)}
