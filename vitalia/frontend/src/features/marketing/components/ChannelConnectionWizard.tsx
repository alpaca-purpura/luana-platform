...
const PROVIDERS: Array<{ slug: ProviderSlug; name: string; description: string }> = [
  {
    slug: "meta_ads",
 ...
 */
export const ChannelConnectionWizard = forwardRef<HTMLDivElement, ChannelConnectionWizardProps>(
  ({ open, onClose, onSuccess, className, _testAuthorizationUrl }, ref) => {
    const [step, setStep] = useState<WizardStep>(1);
    const [selectedProvider, setSelectedProvider] = useState<ProviderSlug | null>(null);
    ...
    const handleAuthorize = () => {
      ...
      const authUrl = _testAuthorizationUrl ?? `/api/v1/vitalia/marketing/channels/${encodeURIComponent(selectedProvider)}/oauth-connect`;
      ...
      >
        <div className="relative w-full max-w-md mx-4 rounded-xl vt-bg-surface border vt-border shadow-2xl">
          ...
          <div className="flex items-center gap-2 px-5 pt-5">
            {([1, 2, 3] as WizardStep[]).map((s) => (
              <div key={s} className="flex items-center gap-1.5">
                ...
                {s < 3 && <div className={cn("h-px w-6 vt-bg-surface-alt", step > s && "vt-bg-success/30")} />}
              </div>
            ))}
          </div>
          ...
          <div className="px-5 py-4">
              ...
              <div data-testid="wizard-step-1" className="flex flex-col gap-2">
                {PROVIDERS.map(({ slug, name, description }) => (
                  ...
                  >
                    ...
                    <div className="text-xs vt-text-muted mt-0.5">{description}</div>
                  </button>
                ))}
              </div>
            )}
            ...
            {step === 2 && (
              <div data-testid="wizard-step-2" className="flex flex-col gap-3">
                <p className="text-sm vt-text">
                  ...
                  {selectedProvider ? MARKETING_COPY.channels.providers[selectedProvider] : "tu canal"}.
            ...
            {/* Step 3: Success */}
            {step === 3 && (
              <div data-testid="wizard-step-3" className="flex flex-col gap-3 items-center py-4">
          ...
          <div className="flex items-center justify-end gap-2 border-t vt-border-soft px-5 py-3">
            ...
            {step === 2 && (
              ...
              >
                {isSyncing ? MARKETING_COPY.channels.syncingLabel : "Verificar conexión"}
              </button>
            )}
