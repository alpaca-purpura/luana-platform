 ...
 */
export const ChannelDetailSidebar = forwardRef<HTMLDivElement, ChannelDetailSidebarProps>(
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {channelData.syncState.lastSuccessAt && (
                      <div className="rounded-md vt-bg-surface-alt p-2.5 flex flex-col gap-0.5">
                        ...
                        <span className="text-xs font-medium vt-text tabular-nums">
                          {new Date(channelData.syncState.lastSuccessAt).toLocaleDateString("es-419", {
                            day: "2-digit",
                            month: "short",
                            hour: "2-digit",
                            minute: "2-digit",
                    ...
                    {channelData.syncState.accountId && (
                      <div className="rounded-md vt-bg-surface-alt p-2.5 flex flex-col gap-0.5">
                        ...
                        <span className="text-xs font-mono vt-text">{channelData.syncState.accountId}</span>
                      </div>
                    )}
                  </div>
                ...
                {top3Campaigns.length === 0 ? (
                  ...
                  <ul className="flex flex-col gap-2">
                    {top3Campaigns.map((campaign: ChannelMetricRow) => (
                      <li
                        key={campaign.campaignId ?? campaign.campaignName ?? campaign.metricDate}
                      ...
                      >
                        ...
                        <div className="mt-1 flex gap-3 text-xs vt-text-muted tabular-nums">
                          {campaign.impressions !== null && (
                            <span>{campaign.impressions.toLocaleString("es-419")} impresiones</span>
                          )}
                          {campaign.clicks !== null && (
                            <span>{campaign.clicks.toLocaleString("es-419")} clics</span>
                          )}
