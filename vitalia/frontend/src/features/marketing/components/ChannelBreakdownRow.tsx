 ...
 */
export const ChannelBreakdownRow = forwardRef<HTMLDivElement, ChannelBreakdownRowProps>(
  ({ provider, className }, ref) => {
    ...
    if (isLoading) {
      return (
        <div
          ...
          aria-busy={true}
        >
          <div role="status" aria-label={MARKETING_COPY.ui.loading} className="animate-pulse flex items-center gap-3 w-full">
    ...
    return (
      <>
        ...
        >
          {/* Provider name */}
          <span className="text-sm font-medium vt-text flex-1 truncate">{providerName}</span>
          ...
          {isError && (
            ...
            >
              {isSyncing ? MARKETING_COPY.channels.syncingLabel : MARKETING_COPY.ui.retry}
            </button>
          )}
