...
const MICROCOPY_PATH = join(ROOT, "src", "features", "vitalia", "config", "microcopy.ts");
...
const SPANISH_INDICATOR_WORDS = [
  ...
  "Crear", "Editar", "Eliminar", "Ver", "Cargar", "Subir", "Descargar",
...
describe("Vitalia FE — user-facing strings in microcopy.ts SSoT (FE-A2b)", () => {
  it("microcopy.ts file exists and exports at least 6 namespaces", () => {
    if (!existsSync(MICROCOPY_PATH)) {
      console.log("[SKIP] microcopy.ts not found — skipping test_no_hardcoded_strings");
      return;
    }
    ...
    const missingExports = requiredExports.filter((name) => !source.includes(`export const ${name}`));
    expect(missingExports, `microcopy.ts missing expected namespace exports: ${missingExports.join(", ")}`).toHaveLength(0);
  });

  it("component files do not inline prominent Spanish user-facing copy", () => {
    if (!existsSync(COMPONENTS_DIR)) {
      console.log("[SKIP] components directory not found — skipping test_no_hardcoded_strings");
      return;
    }
    ...
    for (const absPath of tsxFiles) {
      ...
      const textMatches = Array.from(sourceWithoutComments.matchAll(JSX_TEXT_CONTENT_PATTERN));
      const foundInlineSpanish = textMatches.filter((match) =>
        SPANISH_INDICATOR_WORDS.some((word) =>
      ...
      if (foundInlineSpanish.length > 0) {
        ...
        const importsMicrocopy = source.includes("from") && source.includes("microcopy");
        if (!importsMicrocopy) {
          violations.push(
