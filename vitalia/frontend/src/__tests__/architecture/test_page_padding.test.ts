...
const ALLOWED_PADDING_CLASSES = new Set([
  ...
  "sm:p-6", "md:p-8", "lg:p-8",
  "sm:px-6", "md:px-8", "lg:px-8",
]);
...
describe("Vitalia FE — consistent padding tokens in studio section pages (FE-A8)", () => {
  it("page.tsx and layout.tsx files have no inline style padding/margin", () => {
    if (!existsSync(APP_DIR)) {
      console.log("[SKIP] src/app/ directory not found — skipping test_page_padding");
      return;
    }

    const pageFiles = collectPageFiles(APP_DIR);
    if (pageFiles.length === 0) {
      console.log("[SKIP] No page.tsx / layout.tsx files found — skipping inline style check");
      return;
    }
  ...
  it("studio section client components use consistent Tailwind padding classes", () => {
    if (!existsSync(SRC)) {
      console.log("[SKIP] src/ directory not found — skipping studio section padding check");
      return;
    }

    const studioFiles = collectStudioSectionFiles(SRC);
    if (studioFiles.length === 0) {
      console.log("[SKIP] No studio section files found — skipping consistent padding check");
      return;
    }
