 ...
 * Never use Shadcn <Tabs> internas to group sub-secciones — that is Nivel 4 anti-pattern.
 *
 * URL pattern: /{tenantId}/{agent}/{subtab}/{subsubtab}
 * Active subsubtab derived from URL segment [3] — no client state needed.
 *
 * Returns null when:
 * - No valid agent/subtab from URL (extractAgentFromPath / extractSubtabFromPath return null)
 * - AGENT_SUBSUBTABS has no entry for the current agent.subtab combo
 *
 * Accessibility:
 * - role="tablist" on <nav> (ARIA tablist pattern — navigation bar NOT tab panel body)
 * - role="tab" on each button with aria-selected + aria-current for active
 * - Roving tabindex: only one tab has tabIndex=0 at a time
...
import { AGENT_SUBSUBTABS, type SubSubTabMeta } from "@/lib/shell-routes";
 ...
 */
function extractSubSubTabFromPath(
 ...
 * Returns null for all other agent.subtab combos.
 */
export function SubSubTabsBar() {
  ...
  const subsubtabsKey =
    activeAgent && activeSubtab
      ? (`${activeAgent}.${activeSubtab}` as `${string}.${string}`)
      : null;
  ...
  // Return null when no sub-sub-tabs for this agent.subtab combo
