...
const MOCK_ATTRIBUTION = {
  origins: [
    {
      origin: "sales_agent",
      leads: 180, qualified: 90, conv_listo: 45, reservations: 38, adoption: 28, value_cents: 95000,
    },
    {
      origin: "walk_in",
      leads: 80, qualified: 50, conv_listo: 30, reservations: 25, adoption: 20, value_cents: 55000,
    },
    {
      origin: "phone_manual",
      leads: 50, qualified: 30, conv_listo: 15, reservations: 22, adoption: 14, value_cents: 35000,
    },
    {
      origin: "proactive_outbound",
      leads: 30, qualified: 10, conv_listo: 5, reservations: 10, adoption: 5, value_cents: 20000,
    },
  ],
  totals: {
    origin: "total",
    leads: 340, qualified: 180, conv_listo: 95, reservations: 95, adoption: 67, value_cents: 205000,
  },
...
const MOCK_STAGE_DETAIL = {
  ...
  kpis: [
    ...
    { key: "cpc", label: "CPC promedio", value: 850, unit: "currency", currency: "ARS" },
  ],
  trend_data: [],
};
...
async function setupAllMocks(page: import("@playwright/test").Page) {
  await page.route("**/api/v1/vitalia/marketing/bowtie/summary**", (route) => {
    void route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(MOCK_BOWTIE_SUMMARY) });
  });
  await page.route("**/api/v1/vitalia/marketing/recommendations**", (route) => {
    void route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(MOCK_LUCAS_RECOMMENDATIONS) });
  });
  await page.route("**/api/v1/vitalia/marketing/stages/**", (route) => {
    void route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(MOCK_STAGE_DETAIL) });
  });
  await page.route("**/api/v1/vitalia/marketing/attribution**", (route) => {
    void route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(MOCK_ATTRIBUTION) });
  });
  await page.route("**/api/v1/vitalia/marketing/referrals**", (route) => {
    void route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ referrals: [], total: 0 }) });
  });
  await page.route("**/api/v1/vitalia/marketing/channels**", (route) => {
    void route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ channels: [] }) });
  });
}
...
async function assertNoCriticalA11yViolations(
  ...
  if (!AxeBuilder) {
    test.skip(true, "@axe-core/playwright no instalado — instalar con: npm i -D @axe-core/playwright");
    return;
  }
...
test.describe("Marketing — WCAG 2.1 AA accessibility (axe-core)", () => {
  ...
  test("V-MK-A11Y-01: /marketing tiene 0 violaciones WCAG 2.1 AA críticas/serias", async ({
    ...
    await assertNoCriticalA11yViolations(page, "/marketing (tab attraction, estado cargado)");
  });
  ...
  test("V-MK-A11Y-03: Stage tablist tiene role=tablist + 5 tabs con aria-selected", async ({
    ...
    await expect(mp.stageTablist).toHaveAttribute("aria-label", "Etapas del embudo");

    // 5 tabs con role="tab"
    await expect(mp.stageTabs).toHaveCount(5);

    // Exactamente 1 tab tiene aria-selected="true" (el activo)
    const selectedTabs = mp.stageTabs.filter({ has: page.locator('[aria-selected="true"]') });
    await expect(selectedTabs).toHaveCount(1);

    // Todos los tabs tienen aria-controls apuntando a un panel
    for (const slug of ["attraction", "qualification", "reservation", "adoption", "expansion"]) {
      const tab = mp.stageTab(slug as import("../../pages/marketing.page").MarketingStageSlug);
  ...
  test("V-MK-A11Y-04: tab Reserva (reservation) tiene 0 violaciones + tabla de atribución accesible", async ({
    ...
    const table = page.locator('[role="tabpanel"][id="stage-panel-reservation"] table');
    ...
    await assertNoCriticalA11yViolations(page, "tab Reserva con Attribution Matrix");
  });
  ...
  test("V-MK-A11Y-05: modal de detalle Lucas tiene role=dialog + aria-modal + aria-labelledby", async ({
    ...
    await assertNoCriticalA11yViolations(page, "modal de detalle Lucas abierto");
