...
test.describe("Treatment Followup — Empty state Centro Mindful (CL)", () => {
  ...
  test("V-V-14: no milestone timeline rendered in empty state", async ({
    ...
    await expect(page.getByText(/cirugía|cirug[íi]a|sutura|corona/i)).not.toBeVisible();
  });
  ...
  test("V-V-14: page renders without error boundary", async ({
    ...
    await expect(page.getByText(/algo sali[oó] mal|error inesperado/i)).not.toBeVisible();
  ...
  test("V-V-14: CTA to start treatment available in empty state", async ({
    ...
    const ctaBtn = page
      .getByRole("button", { name: /ver citas|iniciar tratamiento|agendar cita/i })
    ...
    if (!ctaVisible) {
      ...
      await expect(
        page.getByText(/sin tratamientos activos/i)
