...
test.describe("@rule-contact-masked SC-9 — accessibility: keyboard + aria + axe", () => {
  ...
  test("SC-9 — search input is focusable (first interactive element)", async ({ page }) => {
  ...
  test("SC-9 — conversation item is focusable via keyboard", async ({ page }) => {
  ...
  test("SC-9 — Esc in composer returns focus to thread", async ({ page }) => {
    ...
    // Focus should return to thread area
    const threadFocused = await page.locator('[aria-live="polite"]').first().evaluate(
  ...
  test("SC-9 — conversation list has accessible region label", async ({ page }) => {
