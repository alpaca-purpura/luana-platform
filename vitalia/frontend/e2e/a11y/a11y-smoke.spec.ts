 ...
 *   - /onboarding/wizard
 *
 * Página escaneada (con autenticación):
 *   - / (dashboard)
 *
...
authTest.describe(
  "SC-14 — A11y axe smoke (dashboard autenticado — vitalia-auth-base-functional)",
  () => {
    authTest(
      "SC-14 / (dashboard) autenticado pasa axe critical+serious",
      async ({ authedPage }) => {
        ...
        expect(
          criticalOrSerious,
          `Violaciones axe critical/serious en / (dashboard):\n${JSON.stringify(criticalOrSerious, null, 2)}`
        ).toHaveLength(0);
      }
    );
  }
