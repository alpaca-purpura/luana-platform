...
async function setupAuroraMocks(page: Page): Promise<void> {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          sections: {
            ...
            contact: { address: "Av. Corrientes 1234, CABA", phone: "+54-11-4567-8900" },
            team: { doctors: AURORA_FIXTURE.doctors },
            testimonials: [
              { quote: "Excelente atención, muy profesionales", author: "María R.", rating: 5 },
            ],
          },
        }),
      });
  ...
  await page.route("**/api/v1/vitalia/bookings/available-slots**", async (route) => {
    await route.fulfill({
      ...
      contentType: "application/json",
      body: JSON.stringify([
        { slot_iso: "2026-05-20T10:00:00Z", doctor_id: "dr-martinez", doctor_name: "Dr. Martínez", available: true },
        { slot_iso: "2026-05-20T11:00:00Z", doctor_id: "dr-gonzalez", doctor_name: "Dra. González", available: true },
        { slot_iso: "2026-05-21T09:00:00Z", doctor_id: "dr-martinez", doctor_name: "Dr. Martínez", available: true },
      ]),
    });
  });
  ...
  await page.route("**/api/v1/vitalia/treatments/*/followup", async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        status: "active",
        milestones: [
          { day: 0, name: "Cirugía", status: "done", date_iso: "2026-05-01T09:00:00Z" },
          { day: 5, name: "Control", status: "done", date_iso: "2026-05-06T10:00:00Z" },
          { day: 14, name: "Sutura", status: "upcoming", date_iso: "2026-05-15T09:00:00Z" },
          { day: 90, name: "Corona", status: "pending", date_iso: null },
        ],
        next_action: { type: "sutura_removal", date_iso: "2026-05-15T09:00:00Z" },
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        events: [
          ...
          { id: "evt-2", event_type: "pii_detected", severity: "warning", created_at: "2026-05-09T14:00:00Z" },
          { id: "evt-3", event_type: "prompt_injection_blocked", severity: "critical", created_at: "2026-05-08T08:00:00Z" },
        ],
        breakdown: {
          pii_detected: 3,
