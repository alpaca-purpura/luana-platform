/**
 * chris-input.md parser + serializer (round-trip idempotente).
 *
 * Schema: docs/process/chris-input-protocol.md.
 *
...
const CONV_CHRIS_RE = /^### (\d{4}-\d{2}-\d{2} \d{2}:\d{2}) · 🧑 chris$/;
...
export function parseChrisInput(content: string): ChrisInput {
  ...
  const frontmatter = parsed.data as ChrisInputFrontmatter;
  ...
  if (idxNotes === -1 || idxRefs === -1 || idxConv === -1) {
    throw new Error(
      'chris-input.md malformado: faltan una o más de las 3 secciones (💭 Notas / 📎 Referencias / 💬 Conversación)'
    );
  }
...
function isPlaceholderNote(text: string): boolean {
  ...
  return (
    ...
    lower.includes('chris escribe aqui') ||
    lower.includes('chris escribe aquí')
  );
}
...
function parseConvSection(lines: string[]): ConvEntry[] {
  ...
  for (const line of lines) {
    ...
    const chrisMatch = CONV_CHRIS_RE.exec(line);
    if (chrisMatch) {
      flush();
      current = {
        timestamp: chrisMatch[1],
        author: 'chris',
        text: '',
      };
      continue;
...
export function serializeChrisInput(data: ChrisInput): string {
  ...
  for (const entry of data.conversation) {
    if (entry.author === 'chris') {
      body.push(`### ${entry.timestamp} · 🧑 chris`);
    } else {
  ...
  const frontmatter: ChrisInputFrontmatter = {
    ...data.frontmatter,
...
export async function appendConversationEntry(
  ...
  try {
    ...
    data = parseChrisInput(raw);
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === 'ENOENT') {
      throw new Error(`chris-input.md no existe en ${absPath} · skill debe crearlo primero`);
    }
    throw err;
  }
