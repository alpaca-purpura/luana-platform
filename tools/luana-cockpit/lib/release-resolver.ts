...
/** Lee un release YAML por brand + release_id */
export async function readRelease(brand: string, releaseId: string): Promise<Release> {
  const absPath = path.join(releasesPath(brand), `${releaseId}.yaml`);
...
/** Lista todos los releases del brand ordenados por `order` ASC */
export async function listReleases(brand: string): Promise<Release[]> {
  let files: string[] = [];
  try {
    files = await listFiles(releasesPath(brand));
  } catch {
    return [];
  }
 ...
 * checkpoint.md de disco. Maneja stories live (`{brand}/docs/product/stories/`)
 * y archived (`{brand}/docs/archive/{year}/stories/`).
 */
export async function loadStoryStatesForRelease(release: Release): Promise<StoryState[]> {
  const states: StoryState[] = [];
  const liveBase = storiesPath(release.brand);
  const currentYear = new Date().getFullYear();
  const archiveBase = archivePath(release.brand, currentYear);
...
export async function mapStoryToRelease(
  ...
  const currentYear = new Date().getFullYear();
  const archiveBase = archivePath(brand, currentYear);
