...
describe('readCapability · passthrough dimensiones v3.2 (F0 cap-levels)', () => {
  ...
  it('preserva scenarios, business_rules, access y related_capabilities del YAML', async () => {
    ...
    const yaml = [
      ...
      '    added_in_story: vitalia-fase2-adrian-inbox',
      ...
      '    code_ref: vitalia/backend/src/modules/vitalia/inbox/api/router.py',
      '    severity: high',
      'access:',
      '  entry_points:',
      '    - path: /adrian/inbox',
      ...
      '  depends_on: [shell-organism.shell-vitalia]',
      '---',
    ...
    expect(cap.related_capabilities?.depends_on).toContain('shell-organism.shell-vitalia');
  });
});
