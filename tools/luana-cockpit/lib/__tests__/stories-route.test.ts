 ...
 * `{brand}/docs/archive/{year}/stories/` y DEBEN aparecer (is_archived: true).
 *
 ...
 * hubiera 29 stories archivadas. Fix: `archiveRootPath(brand)` → `…/archive`.
 *
...
function makeReq(brand: string) {
  const req = new Request(`http://localhost:3000/api/stories?brand=${brand}`);
  return Object.assign(req, { nextUrl: new URL(req.url) });
}

beforeEach(() => {
  root = mkdtempSync(path.join(tmpdir(), 'luana-stories-'));
...
describe('GET /api/stories — archived done stories', () => {
  it('incluye stories done archivadas con is_archived: true', async () => {
    ...
    const done = body.stories.find((s) => s.story_id === 'vitalia-done-example');
