...
beforeEach(() => {
  root = mkdtempSync(path.join(tmpdir(), 'luana-locks-'));
...
describe('readActiveSessions', () => {
  it('parsea un build-claim vivo (PID actual) con story + lane + bucket', async () => {
    writeLock(
      ...
      `${process.pid} dev-team 2026-05-28T23:00:00-05:00 vitalia-fase2-adrian-embudo A code:scheduling`
    );
    ...
    expect(sessions[0]).toMatchObject({
      pid: process.pid,
      skill: 'dev-team',
      storyId: 'vitalia-fase2-adrian-embudo',
      lane: 'A',
...
describe('sessionsByStory', () => {
  it('mapea story_id → sesión y omite locks sin story (docs)', async () => {
    ...
    writeLock(
      'docs.lock',
      `${process.pid} po-ux 2026-05-28T23:00:00-05:00 — chris docs`
    );
