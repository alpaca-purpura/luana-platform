/**
 * GET   /api/stories/{id}?brand=...  → { story }
 * PATCH /api/stories/{id}?brand=...  body: { fields?: {...} | field, value } → { story }
 *
 * PATCH solo permite editar fields editables Chris (cockpit-permissions.md):
...
async function findStoryPath(brand: string, storyId: string): Promise<string | null> {
...
async function loadStory(storyDir: string, brand: string): Promise<Story> {
...
export async function GET(
  ...
  const storyDir = await findStoryPath(brand, id);
  if (!storyDir) {
    return errorResponse('story no encontrada', 404, { story_id: id, brand });
  }

  try {
    const story = await loadStory(storyDir, brand);
  ...
  if (forbidden.length > 0) {
    return errorResponse(
      ...
      403,
      {
        ...
        reason:
          'estos fields los gestionan las skills (/po-ux, /architect, /dev-team, /pm-{brand}). Modifícalos invocando la skill correspondiente.',
      }
    );
  }
