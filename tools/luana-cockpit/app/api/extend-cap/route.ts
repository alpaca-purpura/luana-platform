 ...
 *  }
 *  → { storyId, slug, checkpointPath, chrisInputPath }
 *
 * Crea checkpoint.md + chris-input.md declarando parentCap + capChangeType.
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const storyDir = path.join(storiesPath(data.brand), data.newStorySlug);
  try {
    await stat(storyDir);
