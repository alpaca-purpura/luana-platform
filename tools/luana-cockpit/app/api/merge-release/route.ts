 ...
 *  body: { brand, releaseId, confirmFinal: boolean, verified?: boolean, verificationNote?: string }
 *
 * Gate de shipped (release-protocol.md § 5): un release pasa a `shipped` SOLO si
 *   (a) todas sus stories están en {done, dropped}  Y
 *   (b) Chris confirma la PRUEBA DE COMPORTAMIENTO verde (verified=true) — corrió
 ...
 * vía /pm-{brand}. El "pase a producción" es OTRO eje (futuro · ver § 8).
 */
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const { brand, releaseId, confirmFinal, verified, verificationNote } = parsed.data;
  ...
  // Gate de comportamiento: shipped requiere confirmación explícita de Chris de que
  ...
  return NextResponse.json({
    ...
    executed: true,
    release,
    note: 'Release marcado shipped (base sólida verificada). El git mv de las stories al archive (si falta) lo haces manual o vía /pm-{brand}. El "pase a producción" es otro paso (futuro).',
  });
}
