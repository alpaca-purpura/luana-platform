 ...
 *   L4 capability-desfasada → cap-doctor por marca              (link /drift · per-brand)
 *
 * Este endpoint hace el "monitor único": L1+L3 traen sus items (mismo lifecycle),
 * L2 trae un conteo del home transversal, L4 queda como link (es per-marca y ya
 * tiene su vista `/drift`). Transversal — NO brand-scoped. El cockpit solo LEE;
...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    const body: CilBoardResponse = {
      ...
      l4: {
        source: 'cap-doctor (por marca)',
        link: '/drift',
        note: 'Capabilities desfasadas se monitorean por marca en la vista Drift.',
      },
    };
