 ...
 *  FormData: { file: Blob, brand: string, storyId: string, comment?: string }
 ...
 *   - Max size 10MB
 *   - Escribe blob a {brand}/docs/product/stories/{storyId}/refs/{filename}
 *   - Appendea entry a chris-input.md sección Referencias
 */
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (typeof brand !== 'string' || !brand) {
    return errorResponse('campo "brand" requerido', 400);
  }
  ...
  // Appendea entry a chris-input.md (si existe)
  ...
  try {
    ...
    // chris-input.md no existe o falla parse · OK · upload sigue válido sin registrar entry
  }
