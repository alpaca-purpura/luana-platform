 ...
 *
 * Permitido:
 *   - {brand}/docs/** (docs producto, archive, learnings)
 *   - tools/luana-cockpit/**
 *   - .claude/**
 *   - docs/**
 */
function isWhitelistedRelPath(relPath: string): boolean {
  ...
  // Brand-scoped: solo dentro de docs/
  if (KNOWN_BRAND_SLUGS.includes(first)) {
    return segments[1] === 'docs';
  }

  // Cross-cutting permitido
  if (first === 'tools' && segments[1] === 'luana-cockpit') return true;
