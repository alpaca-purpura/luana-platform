 ...
 *    release: string
 *  }
 *  → { storyId, checkpointPath, chrisInputPath }
 *
 * Crea una story nueva basada en una done. Auto-fill parent_story + appendea
 * ref "story-ref" al chris-input.md de la nueva ("Spawned from parent: X done").
 */
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const parent = await findParentStory(data.brand, data.parentStoryId);
  if (!parent) {
  ...
  const newDir = path.join(storiesPath(data.brand), data.newStorySlug);
  try {
    await stat(newDir);
  ...
  try {
    ...
    // Appendea ref a chris-input
    await appendStoryRefToChrisInput(
      result.chrisInputPath,
      data.parentStoryId,
    ...
    return NextResponse.json({
      ...
      chrisInputPath: result.chrisInputPath,
    });
  } catch (err) {
